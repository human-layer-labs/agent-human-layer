...
| LC-11 | `risk-levels.md`, database minimum floor; `ahl-flow.md`, DB boundary row | DB/schema and live-data cases automatically received a higher legacy floor or separate route. | Protect DB/live state with a separately protected requirement and suitable release/authority controls. | `boundary-policy.md`, `release-gate-policy.md`, `authorization-policy.md` | Exact Boundary derivation and Release/Authorization consumption; no automatic Level floor. | S049 | RETIRE | The numeric/domain floor is obsolete anatomy; the protected-state requirement is preserved. |
